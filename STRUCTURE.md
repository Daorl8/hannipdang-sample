...
- **폰트(시안=CDN)**: 큰 헤딩(h1·h2) **Dongle 700(둥근 버블, `--display`)** + 소제목·UI·본문 Pretendard(`--serif`=`--sans`) + Gaegu(손글씨 키커·포인트). ⚠️Dongle 단일 웨이트라 h1/h2 크기 1.3x·행간 타이트 보정, 소제목은 Pretendard 유지. 납품 시 서브셋 self-host.
